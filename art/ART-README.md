...
Every model in `assets/entity_models/` is **generated by a Python script run inside Blender**,
...
- What they produce lives in `assets/entity_models/`, in the same folder as its script.
  ...
  already routed there by `.gitattributes`).

## Where models live

Every model of a thing in the game (an animal, a plant, a villager, a yokai, and later
rocks and furniture) has its script under `art/entity_models/` and its `.glb` in the
same folder under `assets/entity_models/`:

```
art/entity_models/
  living/animals/        fox, hare, cat, rat, crane, boar, deer, bear, horse, ox,
                         chicken, dog, tanuki, macaque, wolf, pheasant, heron
  living/human/          villagers
  flora/trees/           maple, sakura, hinoki, redpine, sugi
  flora/bamboo/          bamboo, sasa
  flora/shrubs/          azalea, aoki
  flora/ground/          shida, kusa, kuzu
  spiritual/yokai/humanoid/, spiritual/yokai/nonhumanoid/
```

Folders group things the way a villager of the time would have: bamboo and sasa have
their own folder because 竹 was neither tree nor grass (「木にもあらず草にもあらず」), and
笹 is small bamboo. A script finds the shared code by walking up to the folder that holds
`loft.py`, so a new folder at any depth needs no changes elsewhere. `art/tools/rebuild.sh`
finds a model's script from the folder its `.glb` sits in.
...
blender -b --python art/entity_models/flora/trees/maple.py -- \
    --version 00 --colour a --season summer \
    --out assets/entity_models/flora/trees/maple-00-a-summer.glb

# add a preview sheet (front, side, top and three-quarter views in one PNG)
blender -b --python art/entity_models/flora/trees/sakura.py -- --season spring --out /tmp/x.glb --renders <dir>
```
...
- **Every `.glb` embeds the whole palette texture.** Any palette change (even adding a
  ...
  `art/tools/check.sh <dir>`, and if it reports `ALL MATCH`, copy `<dir>/rebuild/`
  over `assets/entity_models/` (`cp -r <dir>/rebuild/. assets/entity_models/`).
...
| `rebuild.sh <dir>` | rebuilds every committed model into `<dir>`, working out each one's script from its folder and its flags from its file name, and keeping the folders. `check.sh` uses it; on its own it's how to re-export everything after a palette change. |
...
cp -r /tmp/art-check/rebuild/. assets/entity_models/  # after a palette change: take the re-exports
blender -b --python art/tools/ground.py -- assets/entity_models/flora/trees/sugi-00-a-summer.glb
```
