...
├─ crates/                 its members, one directory each: see `CRATE_MANIFEST.md`
├─ CRATE_MANIFEST.md       the tree of crates, and what each one is for
├─ scripts/run.sh          launches the app; what a desktop entry points at. Finds the display itself, logs to ~/.cache/murabito/run.log; `--debug` for the debug build
├─ scripts/probe.sh        watches the debug build's server, redrawing every half second
├─ scripts/tree.sh         prints the tree of kinds as the running world has it
├─ ai/                     minds that run outside the game, in Python: one uv project each
│  └─ midbrain/            the slow mind; `uv run board` watches every body, `uv run order` tells one what to want
├─ TODO.md                 what's queued for the main project: modules to come back, loose ends, housekeeping
├─ docs/                   project docs (no specs or plans; see below)
│  ├─ HANDOFF.md           where the main project stands, for a fresh session
│  └─ *_readme.md          design briefs (hex units, movement, actions, perception, debug, all things, ai): what is implemented and what is still design
├─ _Archives/
...
The repo root is where the Murabito game lives, in Rust on Bevy: a Cargo workspace whose members sit under `crates/`. It is being rebuilt one module at a time: the first Bevy attempt grew tangled and was archived to `_Archives/Bevy-Try-1/`. `docs/HANDOFF.md` says where the rebuild stands and `TODO.md` what is queued. The user drives it; AI assists.
...
- **Each module is its own library crate under `crates/`.** Its `pub` items are its whole API and its `[dependencies]` are its whole wiring: a crate can't reach what another doesn't export, and Cargo refuses dependency cycles. `murabito` is the app and the only binary. `CRATE_MANIFEST.md` is the tree of crates and what each is for; add a crate there when you add one. Members are listed in the root manifest (`cargo new` adds a new one itself; a glob can't cover a group directory such as `crates/action/`), inherit `version`, `edition` and `publish = false` from `[workspace.package]` in the root manifest, and are never published.
...
- **Settings persist through `murabito_settings`, and only the app registers them.** `main.rs` adds `UserDataPlugin` first, `SettingsPlugin` second, and after the plugin list one `.persist::<T>("key")` per settings resource; the file is `~/.config/murabito/settings.yaml`, one top-level key each. A settings type derives `Serialize`/`Deserialize` with `#[serde(default)]`, so a field the file lacks takes its default. A `Bind` is one word in the file (`KeyW`, `MouseLeft`, `Mouse7`) and a `Binds` a list of them; Bevy's `serialize` feature is on for the key and button names. A file or section the game can't read is copied to `settings-<moment>.bak` before being replaced.
...
- **Simulation runs on `FixedUpdate`, 64 Hz; presentation on `Update`.** Movement and everything that changes the world's state tick in `FixedUpdate`, so the same inputs give the same result on every machine. The camera, `place` (which writes a body's `Transform` from its voxel and facing) and drawing run per frame. Inside `FixedUpdate`, `Res<Time>` is the fixed clock.
- **Every thing has a `ThingId`, from `murabito_identity`.** A serial number for life, minted from one counter (`NextThingId`) and never reused, stable across a save as long as the counter is saved with the world; Bevy's `Entity` is only the handle for this run. The kinds stamp it: an observer on `AllThings`, the root, mints one at spawn unless the spawner gave one. Anything with a place that lacks one is a spawner's mistake, and `look` panics saying so rather than leaving it unseen. Whatever remembers a thing across ticks or saves keys on the `ThingId`; whatever acts on it now uses the `Entity`.
- **Every thing in the world is a kind from `murabito_kinds`.** A kind is a unit component whose `#[require]` is its parent and its members (`Fox` requires `Beast`, its `Locomotion` and its `Model`), so spawning a kind inserts the whole chain and the node itself says what it has; a direct `require` wins over an inherited one, which is how a species overrides a tier. One node, one file, in folders that mirror the tree (`…/animal/beast/fox.rs`), English names with the kanji beside them. What a thing *has* is the kind's; where it *is* (`VoxelPosition`, `Facing`, a particular `Model` variant) is given at spawn, beside the kind, and what is given wins. A tangible thing must be given both a `VoxelPosition` and a `Facing`: the tree can't require either (a `require` carries a value, and a place is the instance's), so an observer on `Tangible` panics at spawn if one is missing. `place` needs both, and a thing without one stands at the world origin on screen whatever its voxel says. Only what spawns things depends on `murabito_kinds`. Every node also names itself, `Kind = Kind::at(module_path!())` last in its `require`, and is listed once in the nested roster (`kinds/src/roster.rs`), which is where the debug registration, the tests and the `Ontology` resource (the tree as the world has it; `scripts/tree.sh` prints it) get the nodes; tests hold the `require` chains, the roster, the folders and the labels to one another. Adding a node: `docs/all_things_readme.md`.
- **Whatever pushes onto an `ActionQueue` runs in `murabito_actions::AskingSet`**, so an action asked for on a tick is issued on that tick rather than the next by the scheduler's whim.
- **Doing things takes the shape in `docs/actions_readme.md`.** A *mechanism* crate (`crates/action/mechanisms/*`) exposes an intent component (`Step`, `Turn`) and a `FixedUpdate` system in `MechanismSet` that carries it out and removes it. Every sustained action fills the one `Progress` per entity, in the mechanism's own units (shaku, degrees), by the rules there: `start::<Intent>(cost)`, `advance`, `finish`, overshoot carrying only within a kind. `murabito_actions` owns the `ActionQueue` and is the only place that sequences: anything may push onto the queue, and nothing in it says who did or why. A new kind of action is a new `Action` variant and a new mechanism crate; the queue does not change.
- **A sense is a crate under `crates/perception/senses/`, with its own list in its own shape.** Vision reports `Seen`, a list of `Sighting { entity, id, offset, acuity }` per tick; hearing and smell will report bearings and gradients. Nothing in the perception layer merges the lists or decides anything. A sense's system runs in `murabito_perception::PerceptionSet::Sense`, reads `Occupancy` (what stands where, rebuilt each tick in `Gather`, after the mechanisms), and keeps whatever field it computes on the way private. `Vision` is a member of `Sentient`; a species overrides it or requires `Vision::BLIND`. Everything is opaque until facets return. Design: `docs/perception_readme.md`.
- **The debug build is a Cargo feature, `debug`, off by default**, and the wire is a mirror of memory. `cargo run -p murabito --features debug` (or `scripts/run.sh --debug`) serves Bevy's remote protocol on `127.0.0.1:15702`; without the feature nothing exists, not even the engine's `bevy_remote`. Each crate owns its part: a `debug` feature that forwards to the crates whose types it contains, one `cfg_attr` line per type (`derive(Reflect), reflect(Component)` or `reflect(Resource)`), and its own plugin registering its own types under `#[cfg(feature = "debug")]`; the app's `debug` feature turns them all on plus `murabito_debug`, the server, which depends on no module. A type on the wire looks exactly as it does in memory (a voxel is axial, a one-field tuple struct is flattened, a kind is its full module path); the one hand-written shape is `Occupancy`, a list of `[voxel, things]` pairs, since JSON can't key an object by a struct. A feature-on test pins each shape through the serializer the server uses. Design: `docs/debug_readme.md`.
- **A mind meets a body through the brainstem, and only there.** `murabito_brainstem` owns the words a body can be told (`Short`: stop, face, face a thing, step, each over within a round of the mind; `Sustained`: go to a cell, re-aimed at every step) and a `Brainstem` per sentient that holds one intent, the only thing that pushes onto an `ActionQueue`. Anything may `order` it; a reflex may `preempt` it, cancelling the intent by the reflex's name; a new intent takes the body now, clearing the queue and cutting short what is in flight (`murabito_actions::CutShort`). `previous` records what was done last and how it ended, the mind's only feedback. Reflexes are a catalogue in `murabito_reflexes`, one trigger to one response named `category_reaction_trigger`, answering only a `Short`; a kind gives its repertoire and dials in its `require`. After the senses each tick the brainstem posts every body's `Snapshot` to its `Port`, and drains the port's orders once a tick; `murabito_bridge` serves the port's far ends on `127.0.0.1:15703`, always on, in the protobuf contract `crates/ai/bridge/proto/murabito.proto`, compiled at build time by a crate (`protox`), no `protoc` installed. The midbrain runs outside the game, in Python under `ai/midbrain/`, on its own clock. Design: `docs/ai_readme.md`.
...
- **An overlay is a crate under `crates/ui/` answering one `Overlay` variant.** `murabito_navigation` owns `Overlay` (`None`, `Menu`, `Settings`), a sub-state of `AppState::Paused`, so the world is held still under any overlay and every overlay vanishes when it resumes; an overlay builds itself on `OnEnter` of its variant and tears itself down on `OnExit`, however it is left. Overlays set `Overlay` and never import each other; a new one is a variant there and a crate of its own. Widgets and the look come from `murabito_ui`, which knows no screen and no action: a button carries whatever component the overlay chose. Escape is back one overlay (`NavigationSettings`); Space is the pause (`AppStateSettings`); both persisted.
- **UI text is never a literal.** A text entity carries a `Localized` key and `murabito_i18n` fills in the string for the current `Language`, live. Both catalogues in `assets/locales/` carry the same keys (a test fails otherwise), and a key is added when the text that uses it is. The one exception is a language's own name, `Language::own_name()`, which is never translated.
...
- **Bevy compiles into the binary.** There is no engine install: the `bevy` dependency in `Cargo.toml` is the whole thing. A first build takes about 4.5 minutes (some 550 crates); after that, a change to the project's own code rebuilds in seconds. `target/` runs to tens of GB; moving a warm one between worktrees saves the cold build, as long as `Cargo.lock` and the dev profile match.
...
- **Check visual work by running the app**, `scripts/run.sh`, which finds the display itself and logs to `~/.cache/murabito/run.log`; the user looks and reports. There is no screenshot tool yet. To check the world's data rather than its look, run the debug build and read it with `scripts/probe.sh`.
