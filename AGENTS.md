...
- **Every thing in the world is a kind from `murabito_kinds`.** A kind is a unit component whose `#[require]` is its parent and its members (`Fox` requires `Beast`, its `Locomotion` and its `Model`), so spawning a kind inserts the whole chain and the node itself says what it has; a direct `require` wins over an inherited one, which is how a species overrides a tier. One node, one file, in folders that mirror the tree (`…/animal/beast/fox.rs`), English names with the kanji beside them. What a thing *has* is the kind's; where it *is* (`VoxelPosition`, `Facing`, a particular `Model` variant) is given at spawn, beside the kind, and what is given wins. Only what spawns things depends on `murabito_kinds`.
