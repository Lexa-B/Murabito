...
- **A sense is a crate under `crates/perception/senses/`, with its own list in its own shape.** Vision reports `Seen`, a list of `Sighting { entity, offset, acuity }` per tick; hearing and smell will report bearings and gradients. Nothing in the perception layer merges the lists or decides anything. A sense's system runs in `murabito_perception::PerceptionSet::Sense`, reads `Occupancy` (what stands where, rebuilt each tick in `Gather`, after the mechanisms), and keeps whatever field it computes on the way private. `Vision` is a member of `Sentient`; a species overrides it or requires `Vision::BLIND`. Everything is opaque until facets return. Design: `Docs/perception_readme.md`.
