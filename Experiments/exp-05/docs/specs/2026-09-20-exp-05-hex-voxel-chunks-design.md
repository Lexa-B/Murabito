...
- **Full-depth outer walls.** Side faces toward cells outside the chunk (and all of a guest's outward faces) run down to `bottom_layer`. A wall is two triangles whatever its height, and columns are solid beneath, so steps between detail levels cannot crack. An omitted cell — one whose own, finer chunk is on screen — is treated as not-drawn for this purpose too, since it is exactly a boundary between detail levels; so omitting a cell adds full-depth walls around the hole it leaves rather than simply removing that cell's geometry.
